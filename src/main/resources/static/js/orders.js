    ...
    ordersData.forEach(product => {
        ...
        row.innerHTML = `
            ...
            <td style="text-align: right; display: flex; gap: 5px; align-items: center; justify-content: flex-end;">
                ...
                <button class="btn btn-primary" style="padding: 6px 12px; font-size: 12px;" onclick="event.stopPropagation(); applyOrderAction(${product.id});">Apply</button>
            </td>
        `;
...
function viewProductPhoto(productId) {
    const product = ordersData.find(b => b.id === productId);
    if (!product) return;
...
function checkInStock() {
    ...
    const product = ordersData.find(b => b.id === selectedProductId);
    if (!product) return;
...
function saveNewBud() {
    ...
    const newBud = {
        id: ordersData.length > 0 ? Math.max(...allOrdersData.map(b => b.id)) + 1 : 1,
    ...
    allOrdersData.push(newBud);
    ordersData.push(newBud);
    populateOrdersTable();
    closeAddBudModal();
...
function deleteBud() {
    ...
    if (confirm('Sure want delete this product?')) {
        const index = ordersData.findIndex(b => b.id === selectedProductId);
        if (index > -1) {
            ordersData.splice(index, 1);
            allOrdersData = ordersData.filter(b => allOrdersData.some(ab => ab.id === b.id));
            populateOrdersTable();
...
function removeManufacturer(manufacturer) {
    ...
    const productsUsingManufacturer = ordersData.filter(b => b.manufacturer === manufacturer);
...
function removeCategory(category) {
    ...
    const productsUsingCategory = ordersData.filter(b => b.category === category);
...
// Sort functions for Orders page
function sortByOrdered() {
    ordersData = [...allOrdersData].filter(p => p.orderStatus === 'ordered');
    currentFilters.orderStatus = 'ordered';
    populateOrdersTable();
    updateBreadcrumbFilters();
}

function sortByOrderNow() {
    ordersData = [...allOrdersData].filter(p => p.orderStatus === 'ordernow');
    currentFilters.orderStatus = 'ordernow';
    populateOrdersTable();
    updateBreadcrumbFilters();
}

function sortByDelivered() {
    ordersData = [...allOrdersData].filter(p => p.orderStatus === 'delivered');
    currentFilters.orderStatus = 'delivered';
    populateOrdersTable();
    updateBreadcrumbFilters();
}

function applyOrderAction(productId) {
    const product = ordersData.find(p => p.id === productId);
    if (!product) return;

    const target = document.getElementById(`orderTarget${productId}`).value;
    ...
    // Update order status for order-related actions
    if (target === 'finished' || target === 'ordered' || target === 'ordernow') {
        product.orderStatus = target;

        // Find in allOrdersData and update there too
        const allProduct = allOrdersData.find(p => p.id === productId);
        if (allProduct) {
            allProduct.orderStatus = target;
        }
    }
    ...
    switch(target) {
        case 'finished':
            targetName = 'Finished (Delivered to stock)';
            break;
        case 'ordered':
            targetName = 'Ordered';
            break;
        case 'ordernow':
            targetName = 'Order Now';
            break;
        case 'catalog':
            targetName = 'Catalog';
            break;
...
function deleteProduct() {
    ...
    if (confirm('Sure want delete this product?')) {
        const index = ordersData.findIndex(p => p.id === selectedProductId);
        if (index > -1) {
            ordersData.splice(index, 1);
            allOrdersData = allOrdersData.filter(p => p.id !== selectedProductId);
            populateOrdersTable();
            selectedProductId = null;
            currentPanel = null;
            document.getElementById('rightPanelContent').innerHTML = '';
